...
*------------------------------------------------------------------------------*
	...
	gl run_comments			0	//	Collate and output field comments			
...
*/
*------------------------------------------------------------------------------*
	gl inputfile			"${cwd}/3_checks/1_inputs/1_Baseline/hfc_inputs_Baseline.xlsm"			
	gl corrfile 			"${cwd}/3_checks/1_inputs/1_Baseline/corrections_Baseline.xlsm"		
*	gl recodefile 			"${cwd}/3_checks/1_inputs/1_Baseline/specifyrecode_Baseline.xlsm"	// No recode
...
*/
*------------------------------------------------------------------------------*
	gl rawsurvey			"${cwd}/4_data/2_survey/0_Mock_dataset_baseline/Baseline_Survey_Lactation_Rooms_Scaleup.dta"
	gl preppedsurvey		"${cwd}/4_data/2_survey/1_Baseline/Baseline_survey_prepped.dta"			
	gl checkedsurvey		"${cwd}/4_data/2_survey/1_Baseline/Baseline_survey_checked.dta"	
	gl mastersurvey 		""
	gl trackingsurvey 		""
	...
	gl trackingbc 			""
*	gl commentsdata 		"${cwd}/4_data/2_survey/1_Baseline/comments_data.dta" 
*	gl textauditdata 		"${cwd}/4_data/2_survey/1_Baseline/textaudit_data.dta"
...
*/
*------------------------------------------------------------------------------*
	gl media_folder 		"${cwd}//Users/flavia_ungarelli/Dropbox/Kenya Lactation Rooms/9.SCALEUP/HFC/5_media"
	
**# Output Date Folder
*------------------------------------------------------------------------------*		
	gl folder_date			= string(year(today())) + "-`:disp %tdNN today()'-`:disp %tdDD today()'"
	cap mkdir				"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date"
...
*/
*------------------------------------------------------------------------------*	
	gl id_dups_output 		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/survey_duplicates.dta"
	gl corrlog_output 		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/correction_log.xlsx"
	gl recodelog_output		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/specify_recode.xlsx"
	gl hfc_output			"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/hfc_output.xlsx"
*	gl textaudit_output 	"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/textaudit.xlsx"
*	gl timeuse_output		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/timeuse.xlsx"		
	gl surveydb_output		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/surveydb.xlsx"
	gl enumdb_output		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/enumdb.xlsx"
	gl tracking_output      "${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/tracking.xlsx"
	gl id_dupsbc_output		"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/bc_duplicates.dta"
	gl trackingbc_output	"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/bc_tracking.xlsx"
	gl bc_output			"${cwd}/3_checks/2_outputs/1_Baseline/$folder_date/bc_comparison.xlsx"
...
*------------------------------------------------------------------------------*
	* Required Variables:
	gl key					"key"
	gl skey					"skey"
	gl id 					"unique_teacher_id"													
	gl enum					"calc_enum_name" 										 
	gl enum_id				"enum_id"
	gl subcounty			"subcounty"
	gl bcer					""
	gl bcerteam				""
	gl date					"submissiondate"
	gl school 				"school_name"
	...
	* Optional Variables:
	...
	gl duration_all			"consent_duration modb_duration modl_duration modc_duration modd_duration modf_duration modg_duration modh_duration modi_duration modj_duration modk_duration modm_duration"											
	gl formversion 			"formdef_version"
	gl fieldcomments 		"comments"

	gl consent 				"consented"	
	gl consented_str		"consented_str"
	gl outcome				"s_status"
	gl outcome_str			"s_status_str"
	gl availability			"availability"
	gl available_str		"availability_str"
*	gl textaudit 			""											
	gl keepvars 			"${enum} ${enum_id} ${subcounty} ${school} ${outcome_str} ${consent_str} ${available_str} ${date}"													
...
*/
*------------------------------------------------------------------------------*
	gl cr_dupsheet			"duplicates"							
	gl cr_dupslogsheet		"duplicates corrections"							
	gl cr_othersheet		"corrections"									
...
*/
*------------------------------------------------------------------------------*
	gl dp_vars				"${key} skey phone_number tsc_number full_name"									
	gl dp_keepvars			"${id} ${enum}"													
	gl dp_nolabel 			""													
...
*------------------------------------------------------------------------------*	
	gl ms_vars	 			"_all"												
	gl ms_pr_vars			"${id} ${key} ${date} ${enum} ${consent} modb_b9 modb_b10_filtre modh_h1 modb_b3 survey_type" 						
	gl ms_show 				"0"													
...
*------------------------------------------------------------------------------*	
	...
	gl ol_nolabel			""
	gl numeric_variables	"modb_b7b modb_b10b modb_b10d modd_d27a modd_d27a_male modl_l1 modl_l1b1 modl_l1b2 modl_l1c1 modl_l1c2 modl_l7 modl_l8 modl_l9 modl_l13 modl_l14 modl_l15 modc_c3f3 modc_c8b modd_d5 modd_d6b modd_d7 modd_d8b modd_d9b modd_d10 modd_d12 modd_d15e modd_d19c modd_d20c modd_m11 modd_d23_f modd_d23b_f modd_d30 modd_d31 modd_d32 modd_d33 modd_d1b_male_old modd_d5b_male modd_d5d_male modd_d5e_male modd_d11c_male modd_d14b_male modd_d15d_male modd_m11_male modd_m11b_male modd_m11c_male modd_d23_male modd_d30_male modd_d32_male modf_f2 modf_f3 modf_f5a modh_h12 modh_h13 modi_i6b modi_i6c modi_i7b modi_i8 modj_j4b modj_j6b modj_j7a modj_j7b modk_k5a modm_m5"
	
	gl numeric_variables_pilot "modb_b10b modb_b10d modl_l1 modl_l7 modl_l8 modl_l9 modl_l13"
...
/* FU: add if we have a "tracking survey" that is different from the "master survey".

	gl tr_by 				"subcounty"													
	gl tr_target 			""													
	
	gl tr_keepmaster		"teacher_id f_name m_name s_name subcounty"
	
	gl tr_keeptracking		"teacher_id f_name m_name s_name subcounty"													
	gl tr_keepsurvey 		""													
